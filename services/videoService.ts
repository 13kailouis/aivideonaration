  ...
  try {
    ...
    videos = videos.filter(
      (v) =>
        ...
        v.imageinfo[0] &&
        v.imageinfo[0].mime &&
        v.imageinfo[0].mime.startsWith("video"),
    );
...
export const fetchPlaceholderFootageUrl = async (
  ...
  const query =
    keywords && keywords.length > 0
      ? keywords.join(" ")
      : FALLBACK_FOOTAGE_KEYWORDS[
          Math.floor(Math.random() * FALLBACK_FOOTAGE_KEYWORDS.length)
        ];

  const orientation = aspectRatio === "16:9" ? "landscape" : "portrait";

  let wikiVideo = await fetchWikimediaVideo(query, orientation, duration);
  if (!wikiVideo) {
    // Try up to three random fallback queries if the initial search fails
    for (let i = 0; i < 3 && !wikiVideo; i++) {
      const fallbackQuery =
        FALLBACK_FOOTAGE_KEYWORDS[
          Math.floor(Math.random() * FALLBACK_FOOTAGE_KEYWORDS.length)
        ];
      wikiVideo = await fetchWikimediaVideo(
        fallbackQuery,
        orientation,
        duration,
      );
  ...
  const defaultVideo =
    "https://upload.wikimedia.org/wikipedia/commons/transcoded/0/0f/SD_video_test_pattern.webm/SD_video_test_pattern.webm.mp4";

  return { url: wikiVideo || defaultVideo, type: "video" };
};
...
export const processNarrationToScenes = async (
  ...
  onProgress: (
    message: string,
    valueWithinStage: number,
    stage: "ai_image" | "placeholder_image" | "finalizing",
    current?: number,
    total?: number,
    errorMsg?: string,
  ) => void,
  ...
  const scenes: Scene[] =
    existingScenes && !options.generateSpecificImageForSceneId
      ? [...existingScenes]
      : [];
  ...
  if (options.generateSpecificImageForSceneId && existingScenes) {
    const sceneToUpdateIndex = existingScenes.findIndex(
      (s) => s.id === options.generateSpecificImageForSceneId,
    );
    if (sceneToUpdateIndex !== -1) {
      ...
      const analysisItemForScene = narrationAnalysis.find(
        (item) =>
          item.sceneText === existingScenes[sceneToUpdateIndex].sceneText,
      ) || {
        ...existingScenes[sceneToUpdateIndex],
        duration: existingScenes[sceneToUpdateIndex].duration,
      ...
      console.warn(
        "Scene to update image for not found:",
        options.generateSpecificImageForSceneId,
      );
  ...
  for (let index = 0; index < scenesToProcess.length; index++) {
    ...
    const sceneId =
      options.generateSpecificImageForSceneId || `scene-${index}-${Date.now()}`;
    let footageUrl = "";
    ...
    const duration =
      item.duration > 0
        ? item.duration
        : calculateDurationFromText(item.sceneText);
    ...
    if (options.useAiGeneratedImages && item.imagePrompt) {
      ...
      const imagenResult = await generateImageWithImagen(
        item.imagePrompt,
        sceneId,
      );
      if (imagenResult.base64Image) {
        ...
        imageGenError =
          imagenResult.userFriendlyError ||
      ...
      onProgress(
        `Fetching placeholder video for scene ${index + 1}/${scenesToProcess.length}...`,
        (index + 1) / scenesToProcess.length,
        "placeholder_image",
        index + 1,
        scenesToProcess.length,
    ...
    if (options.generateSpecificImageForSceneId && existingScenes) {
      ...
      if (sceneToUpdateIndex !== -1) {
        ...
        if (item.duration)
          existingScenes[sceneToUpdateIndex].duration = validatedDuration;

        if (imageGenError) {
          // Store error if any for this specific update
